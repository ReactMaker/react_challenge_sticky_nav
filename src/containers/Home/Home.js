...
export default class Home extends Component {
  state = {
    isScrollOverNav: false,
  }

  componentDidMount() {
    window.addEventListener('scroll', this.handleScroll);
    mainOffsetTop = this.main.offsetTop;
  }

  handleScroll = (e) => {
    if (window.pageYOffset >= mainOffsetTop) {
      this.setState({
        isScrollOverNav: true,
      });
    } else {
      this.setState({
        isScrollOverNav: false,
  ...
  render() {
    const { isScrollOverNav } = this.state;

    return (
      <div className={`pageHome ${isScrollOverNav ? 'fixed-nav' : ''}`}>
        <header>
          <h1>A story about getting lost.</h1>
        </header>

        <nav id="main" ref={main => (this.main = main)}>
          <ul>
        ...
        <div className="site-wrap">
          ...
          <img src="http://unsplash.it/400/400" className="align-left slide-in" alt="" />
          ...
          <img src="http://unsplash.it/400/400" className="align-right slide-in" alt="" />
